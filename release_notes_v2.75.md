...
Sax Shop Companion is a desktop toolkit for saxophone repair techs: it generates SVG and G-code for laser-cutting pad materials, packs dies and die holders, keeps reference libraries for key heights, serial numbers, and screw specs, and includes a chromatic strobe tuner and a harmonic tone analyzer. v2.75 is a maintenance release: nesting no longer freezes on large sheets, Scrap Mode picks up pad-list edits mid-session, Frame & Cut puts camera-captured cuts exactly where the frame showed them, and the large-batch optimization prompt is gone.

## Pad Maker

- **Labeled Zones (beta)** (Options > Sizing Rules) — cut each pad size in its own bordered, labeled group so you can tell small discs apart when you're picking them off the bed. Off by default; turn it on and set the size range it applies to (7.0–12.5 mm out of the box).
...
- Nesting preview with per-material approval, edge bias d-pad (cardinal + corner directions, smallest pads first in corners), max fill mode (`size x max`). **(new)** Nesting is quick at every edge-bias setting; a large sheet with 100+ pads and bias set to off used to freeze the app for minutes.
- Custom polygon shapes for irregular leather skins.
- Scrap mode — place pads across multiple irregular pieces, tracking remaining between sheets, with preview, edge bias, and polygons all working together. **(new)** The pad list stays editable during a session: add a size, change a quantity, or delete a line between scraps and the next scrap picks it up, with everything already cut still credited. Previously the session ignored the list until you cleared it.
...
- **(new) The large-batch optimization prompt in Scrap Mode is gone.** Measured on real pad lists it changed material usage by about 1% or less, so the standard nest is now the only nest. If you want the leftover space on a scrap used, add small sizes to the list or an `x max` line.
- **(new) If you use Frame & Cut with camera-captured scraps,** expect the pads to land a few millimetres further from the front edge of the piece than they did before — that's the alignment fix above, not a change in your calibration.
...
- **Your saved Sizing Rules presets still work.** Presets saved before v2.7 simply read as "zones off," and the dropdown will still recognise which one you're in.
