# v2.6 — Per-Material G-code Presets + Machine-Integration Polish

Sax Shop Companion v2.6 refines the v2.5 machine-integration layer and adds a per-material G-code preset library. Save and reload your laser feeds, powers, and passes per material — felt, card, leather, acrylic, basswood — each with its own Load / Save / Rename / Delete bar, shared across the Pad Maker and Tooling settings dialogs. **Frame & Cut now works in Scrap Mode**, so you can capture, frame, and cut irregular scrap pieces one at a time. Plus a reliability fix so the laser recovers from a stuck-alarm state without a power cycle. The rest of the app (Pad Maker, Tooling, Tuner, Tone Analyzer) carries forward unchanged.
...
- **Machine Integration (experimental, opt-in)** — direct USB serial control of a Grbl-compatible laser. Enabled via File > Feature Set > "Experimental: machine integration." Off by default; off entirely if you skip the opt-in. Tested on the Creality Falcon2 Pro 40W; might also work on other Grbl 1.1+ machines.
  ...
  - **Frame & Cut** — third button next to Generate SVG / Generate G-code. Generates in memory, opens the position-the-head dialog (Home Laser, jog cluster, Try Auto Locate when a camera-referenced polygon is loaded), runs a low-power framing loop until you click "Looks Good — Cut!", then streams the cut. Pause / Resume / Stop in real time. **(new)** Now works in **Scrap Mode** — one scrap per click: place a partial batch, frame the captured outline, cut, then re-capture the next piece from the continue dialog, repeating until every pad is placed.
  - **(new) Stuck-alarm recovery** — if a previous run left Grbl in an alarm state, the next stream clears it automatically (`$X` at stream start, a no-op when idle), so a hiccup no longer means power-cycling the Falcon and restarting the app.
...
- **Fully translated** — the entire UI, including this release's per-material G-code preset dialogs, is localized into Spanish, German, French, and Italian. Sax-craft terminology (pad / tampón / tampon / Polster / tampone; basswood / tilo / tilleul / Lindenholz / tiglio; etc.) kept consistent across locales.
