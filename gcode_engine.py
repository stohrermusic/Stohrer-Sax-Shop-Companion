...
def _generate_star_points(cx, cy, outer_r, inner_r, num_points, shape_factor=0.5):
    """
    Generate star/dart pattern points for G-code.

    Uses svg_engine._wave_value — the same wave math as calculate_star_path()
    — but returns a list of (x, y) points instead of an SVG path string, so
    the G-code cut matches the SVG output and the live pad preview exactly.
    ...
    Args:
        ...
        shape_factor: 0.0 = Triangle, 0.5 = Sine, 1.0 = Square
...
def generate_gcode_from_placed(placed, material, sheet_width_mm, sheet_height_mm, filename,
    ...
    def _collect_disc_strokes(pad_size, cx, cy, radius):
        ...
        font_size = eng_cfg.get("engraving_font_size", {}).get(material, 2.0)  # 2.0 matches svg_engine fallback
        ...
        if should_engrave:
            ...
            # `placed` was Y-flipped above (G-code Y-up vs SVG Y-down), so the
            # label goes ABOVE the disc center: cy PLUS the offset. These mirror
            # the SVG formulas in svg_engine.py. No baseline adjustment here —
            # SVG adds font_size*0.35 to convert the visual center into a <text>
            # baseline, but get_text_strokes()/get_filled_text_strokes() are
            # already center-anchored.
...
def generate_die_gcode_from_placed(placed, sheet_width_mm, sheet_height_mm, filename, settings):
    ...
    for pad_size, cx, cy, radius in placed:
        ...
        if engrave_ring:
            ...
            # G-code Y is flipped vs SVG. No baseline adjustment: SVG's
            # +0.35*font converts center→<text> baseline, but the gcode
            # text helpers are already center-anchored.
            if ring_location == "from_outside":
                label_y = cy + (radius - ring_offset)
            else:  # centered
                ring_center_r = (radius + inner_r) / 2
                label_y = cy + ring_center_r
        ...
        if engrave_cutout:
            ...
            if inner_r > font_size:
                label_y = cy  # Centered in inner disc (text helpers are center-anchored)
...
def generate_kerf_test_gcode(material_name, filename, settings, cut_speed, cut_power,
    ...
    width_mm = x_cursor
    # Layout must match generate_kerf_test_svg() in svg_engine.py exactly,
    # so the engraved test piece is the same in both output formats.
    ...
    # Title. The SVG version anchors text at its baseline; the stroke
    # helpers are center-anchored, hence the -0.35*font correction.
    title_text = f"Kerf Test - {material_name}"
    title_y_svg = title_height * 0.7 - 4.0 * 0.35
    ...
    for dia, cx, r in positions:
        label_text = f"{dia:.0f}"
        label_y_svg = title_height + label_height * 0.5 - 3.5 * 0.35
