...
def _filled_circle_scan_strokes(cx_mm, cy_mm, diameter_mm, line_spacing_mm):
    """Build a list of horizontal scan-line stroke segments that filled-
    raster a circle of ``diameter_mm`` centered on (cx_mm, cy_mm).
    ...
    strokes = []
    r = diameter_mm / 2.0
    n_lines = max(1, int(round(diameter_mm / line_spacing_mm)))
    for i in range(n_lines):
        # Position the line at y = cy + offset, where offset spans -r..+r
        # (inclusive) at the center of each scan band.
        t = (i + 0.5) / n_lines  # 0 < t < 1
        y_offset = -r + t * diameter_mm
        y = cy_mm + y_offset
        # Chord length at this y: 2 * sqrt(r^2 - y_offset^2)
        d2 = r * r - y_offset * y_offset
        if d2 <= 0:
            continue
        half = d2 ** 0.5
...
def generate_dot_calibration_gcode(filename, engrave_offset_x_mm,
                                     engrave_offset_y_mm,
                                     grid_cols, grid_rows,
                                     spacing_mm, dot_diameter_mm,
                                     marker_local_xy, marker_diameter_mm,
                                     settings,
                                     engrave_speed=6000, engrave_power=25,
                                     engrave_passes=1, line_spacing_mm=0.15,
                                     air_assist=True):
    """Generate G-code for the dot-pattern laser-head calibration.

    Engraves an N x M grid of filled circles + one larger orientation marker.
    Each dot's position is computed in pattern-local coords, then translated
    by (engrave_offset_x_mm, engrave_offset_y_mm) into machine coords.

    Defaults (6000 mm/min, 25%, 0.15mm scan spacing) match the ChArUco card
    on basswood. With an 8x8 grid of 6mm dots + 1x 10mm marker, the engrave
    takes ~3-4 minutes including rapid moves between dots.

    The pattern centers on whichever (offset_x, offset_y) the caller passes
    in — typically the user's framing position. Caller is responsible for
    homing the laser beforehand so the absolute machine coords are meaningful.
    """
    return_speed = settings.get("gcode_return_speed", 1000)
    overscan_mm = 0
    if settings.get("filled_overscan_enabled", False):
    ...
    # Build strokes for every dot. Local coords first (Y-down convention
    # within the pattern), then translate to machine coords with the Y
    # flip handled at the engine boundary just like the calibration card.
    all_strokes_y_down = []
    pattern_h_mm = (grid_rows - 1) * spacing_mm  # grid extent in Y
    # Pattern bbox accounts for the marker too: it sits past the top-right
    # grid corner, so the bbox includes its full diameter.
    pattern_h_mm = max(pattern_h_mm,
                       marker_local_xy[1] + marker_diameter_mm / 2.0)
    # Grid dots in pattern-local coords (Y goes up = away from machine
    # front; we'll flip to machine Y-up at the translate step).
    for col in range(grid_cols):
        for row in range(grid_rows):
            cx = col * spacing_mm
            cy = row * spacing_mm
            all_strokes_y_down.extend(
                _filled_circle_scan_strokes(
                    cx, cy, dot_diameter_mm, line_spacing_mm))
    # Marker (different size, fully outside the grid).
    all_strokes_y_down.extend(
        _filled_circle_scan_strokes(
            marker_local_xy[0], marker_local_xy[1],
            marker_diameter_mm, line_spacing_mm))

    # Translate to machine coords. Pattern-local already uses the same
    # Y-orientation as machine (Y up = +Y machine), so no flip needed
    # here — the caller's offset is in machine coords directly.
    strokes_machine = [
        [(x + engrave_offset_x_mm, y + engrave_offset_y_mm)
         for (x, y) in stroke]
        for stroke in all_strokes_y_down
    ]

    # Bounds for the header comment. Pattern spans engrave_offset to
    # engrave_offset + max(grid_size, marker_pos + marker_radius).
    max_x_local = max((grid_cols - 1) * spacing_mm,
                      marker_local_xy[0] + marker_diameter_mm / 2.0)
    max_y_local = max((grid_rows - 1) * spacing_mm,
                      marker_local_xy[1] + marker_diameter_mm / 2.0)
    min_x_local = -dot_diameter_mm / 2.0  # leftmost extent of dot at (0, 0)
    min_y_local = -dot_diameter_mm / 2.0
    bounds_min_x = engrave_offset_x_mm + min_x_local
    bounds_min_y = engrave_offset_y_mm + min_y_local
    bounds_max_x = engrave_offset_x_mm + max_x_local
    bounds_max_y = engrave_offset_y_mm + max_y_local

    lines = []
    lines.extend(generate_gcode_header(
        bounds_min_x, bounds_min_y, bounds_max_x, bounds_max_y))
    lines.extend(generate_gcode_layer(
        strokes_machine, engrave_speed, engrave_power, 'DOT',
        overscan_mm=overscan_mm, air_assist=air_assist,
        passes=engrave_passes,
    ))
    lines.extend(generate_gcode_footer(return_speed=return_speed))
