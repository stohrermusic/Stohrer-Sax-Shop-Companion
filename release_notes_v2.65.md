...
Sax Shop Companion is a desktop toolkit for saxophone repair techs: it generates SVG and G-code for laser-cutting pad materials, packs dies and die holders, keeps reference libraries for key heights, serial numbers, and screw specs, and includes a chromatic strobe tuner and a harmonic tone analyzer. v2.65 adds a **Job History** to the Pad Maker — every batch you send to the laser or write to file is logged, and you can pull any past job straight back into the form to run it again. Handy when a customer brings back the same horn, when you need to remember what you cut last Tuesday, or when a cut got interrupted and you want to know exactly what was on the sheet. This release also completes the translations: all four languages are now 100% covered, and some long-standing character-encoding corruption in the German, French, and Spanish catalogs has been repaired.
...
- **Fully translated** — the entire UI is localized into Spanish, German, French, and Italian. Sax-craft terminology (pad / zapatilla / tampon / Polster / tampone; basswood / tilo / tilleul / Lindenholz / tiglio; etc.) kept consistent across locales. **(new)** All four catalogs are now at 100% — the last untranslated strings (the Speed & Power Test washer controls and its warnings) have been filled in, and character-encoding corruption that had crept into a handful of German, French, and Spanish messages is repaired, so accented characters display properly again.
...
- **Job History starts empty.** It logs from the moment you install v2.65 forward; there's no way to reconstruct jobs you cut before this release. The log lives in `job_history.json` in your config folder alongside your presets.
