...
- Scrap mode — place pads across multiple pieces, tracking remaining between sheets
  ...
  - **Large-batch optimization** (≥75 pads): opt-in multistart greedy that tries multiple disc orderings per scrap and keeps the layout that uses the most material, so it never does worse than the standard nest.
