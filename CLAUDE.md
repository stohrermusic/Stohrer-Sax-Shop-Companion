...
- Recording quality: `compute_rolloff_rate()` measures harmonic rolloff (dB/harmonic). Live warning if rolloff > 2.5 dB/H (suppressible per-preset via "Don't show again" checkbox). Stored per session and in fingerprints. Comparison tool warns on mismatch. Note: sopranos naturally read higher rolloff (~2.8-4.0 dB/H) than altos (~1.9-2.3) or tenors (~1.1-2.0) — the threshold may need sax-type-specific adjustment in a future release.
